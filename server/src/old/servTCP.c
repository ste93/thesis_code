...
#if 0
  /* prova  di ricerca indirizzo locale del socket listening */
  {
    int namelen;
    ...
    namelen=sizeof(name);
    n=getsockname(socketfd, (struct sockaddr*) &name , &namelen );
    if(n!=0) 
    {
...
#if 0
  /* prova di ricerca indirizzo locale del socket connesso */
  {
    int namelen;
    struct sockaddr_in name;
    ...
    memset(&name,0,sizeof(name));
    namelen=sizeof(name);
    n=getsockname(newsocketfd, (struct sockaddr*) &name , &namelen );
    if(n!=0) 
    {
    	perror("getsockname failed: ");
		exit(2);
