...
int find_range( node * root, int key_start, int key_end, bool verbose, map<int, int> &returned_keys, map<int, void *> &returned_pointers); 
 ...
 */
record * make_record(int values[], int key) {
	...
	if (new_record == NULL) {
		...
		new_record->sector = last_sect_used;
		last_sect_used++;
	}
	return new_record;
}
